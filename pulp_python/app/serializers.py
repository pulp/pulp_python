...
class DistributionDigestSerializer(serializers.ModelSerializer):
    ...
    """
    type = serializers.CharField(help_text=_("A type of digest: i.e. sha256, md5"))

    digest = serializers.CharField(help_text=_("The digest of the distribution"))
...
class ProjectSpecifierSerializer(serializers.ModelSerializer):
    ...
    name = serializers.CharField(help_text=_("A python project name."))

    version_specifier = serializers.CharField(
        help_text=_(
            "A version specifier, accepts standard python versions "
    ...
    digests = DistributionDigestSerializer(required=False, many=True)
...
class PythonPackageContentSerializer(core_serializers.ContentSerializer):
    ...
    name = serializers.CharField(help_text=_("The name of the python project."))
    version = serializers.CharField(help_text=_("The packages version number."))
    metadata_version = serializers.CharField(help_text=_("Version of the file format"))
    ...
    classifiers = ClassifierSerializer(required=False, many=True)
    ...
    def create(self, validated_data):
        ...
        for classifier in classifiers:
            python_models.Classifier.objects.create(
                python_package_content=PythonPackageContent, **classifier
            )
...
class PythonRemoteSerializer(core_serializers.RemoteSerializer):
    ...
    projects = ProjectSpecifierSerializer(required=False, many=True)
...
class PythonSyncSerializer(serializers.Serializer):
    repository = serializers.HyperlinkedRelatedField(
        ...
        label=_("Repository"),
        error_messages={"required": _("The repository URI must be specified.")},
    )
