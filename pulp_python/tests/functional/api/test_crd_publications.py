...
class PublicationsTestCase(unittest.TestCase, utils.SmokeTest):
    ...
    def test_02_read_publications(self):
        ...
        page = self.client.get(PUBLICATIONS_PATH, params={"repository_version": self.repo["_href"]})
    ...
    def test_03_read_publications(self):
        ...
        page = self.client.get(PUBLICATIONS_PATH, params={"publisher": self.publisher["_href"]})
        self.assertEqual(len(page["results"]), 1)
        for key, val in self.publication.items():
            with self.subTest(key=key):
                self.assertEqual(page["results"][0][key], val)
    ...
    def test_04_read_publications(self):
        ...
        page = self.client.get(PUBLICATIONS_PATH, params={"created": self.publication["created"]})
        self.assertEqual(len(page["results"]), 1)
        for key, val in self.publication.items():
            with self.subTest(key=key):
                self.assertEqual(page["results"][0][key], val)
    ...
    def test_05_read_publications(self):
        ...
        page = self.client.get(PUBLICATIONS_PATH, params={"distributions": distribution["_href"]})
        self.assertEqual(len(page["results"]), 1)
        for key, val in self.publication.items():
            with self.subTest(key=key):
                self.assertEqual(page["results"][0][key], val)
