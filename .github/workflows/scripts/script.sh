...
if [[ "$TEST" = "docs" ]]; then
  ...
  cmd_prefix bash -c "cd pulp_python/docs; make diagrams html; tar -cvf docs.tar ./_build"
  docker cp pulp:/pulp_python//docs/docs.tar docs.tar
...
# Run unit tests.
cmd_prefix bash -c "PULP_DATABASES__default__USER=postgres pytest -v -r sx --color=yes -p no:pulpcore --pyargs pulp_python.tests.unit"
